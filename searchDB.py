...
def maka_table():
	c.execute("CREATE TABLE IF NOT EXISTS words(id INTEGER PRIMARY KEY, unix REAL, words")

def search_data():
	#stop word list from github
	...
	with open(SEARCH_LOCATION, "r", encoding="utf8") as f:
		contents = f.read()
		for item in contents.split ("Searched for") [1:]:
			try:
				#print(item)	
				search_string = item.split ("\">")[1].split("</a>")[0]
				search_date = item.split("<br>")[1].split("</div>")[0]
